...
## Goal
Validate this diff against the approved spec and identify violations of acceptance criteria, constraints, and intent.

## Inputs
- Diff under review
- Spec: `_bmad-output/implementation-artifacts/spec-backend-auth-admin-separate-layout.md`
- Read access to project files referenced in diff

## Spec requirements to enforce
- `/login` must be standalone auth UI with no frontend header/footer.
- `/admin` and admin subpages must use admin shell and not frontend shell.
- Frontend routes like `/` and `/about` must still show normal frontend header/footer.
- Auth redirect behavior for admin unauthorized access must remain intact.
...
- `category` (intent_gap|bad_spec|patch|defer|reject)
- `file`
- `finding`
- `evidence`
- `recommended action`
If fully compliant, return: `No acceptance violations found.`